"""Helper function for neural network train and inference."""
import torch


def weighted_MSE(target, out, weights):
    """Weighted MSE loss to be able to cost the errors at
    different parts of the trajectories differently.

    args:
        target: (tensor with shape=(batch_size, pred_len, pose_size))
        out: (tensor with shape=(batch_size, pred_len, pose_size))
        weights: (tensor with shape=(pred_len))

    returns:
        The scalar weighted MSE loss.
    """

    loss = (target - out) ** 2

    if len(loss.shape) == 3:
        loss = torch.sum(loss, dim=(0, 2))
    else:
        loss = torch.sum(loss, dim=(0))

    return loss @ weights[: len(loss)] / (out.shape[0] * out.shape[1])
...
def MSE2(y_pred1, y_pred2, probs, y_gt):
    ...
    _, mse_det1 = MSE(y_pred1, y_gt)
    _, mse_det2 = MSE(y_pred2, y_gt)
